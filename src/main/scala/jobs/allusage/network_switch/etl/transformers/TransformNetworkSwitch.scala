...
package jobs.allusage.network_switch.etl.transformers

import core.logger.Logger
import utils.SparkUDF

import org.apache.spark.sql.DataFrame
import org.apache.spark.sql.functions._
import org.apache.spark.sql.types.IntegerType

object TransformNetworkSwitch extends Logger {
  def process(df: DataFrame): DataFrame = {
    df
    // .filter(length($"value") >= 180)
    // .filter(substring($"value", 62, 4).rlike("20[0-9][0-9]"))
    // .filter(substring($"value", 66, 2).rlike("[0-1][0-9]"))
    // .filter(substring($"value", 68, 2).rlike("[0-3][0-9]"))
    // .select(
